    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            foreach ($statuses as $status => $status_code) {
                ...
                // Fetch and display users based on status
                $query = "SELECT * FROM custom_property WHERE custom_property_status = '{$status_code}'";
                $result = mysqli_query($conn, $query);

                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<div class='user-details'>";
                    echo "<p>Custom Property Id: {$row['custom_property_id']}</p>";
                    echo "<p>Custom Property Name: {$row['property_name']}</p>";
                    ...
                    foreach ($statuses as $actionStatus => $actionStatusCode) {
                        if ($status !== $actionStatus) {
                            echo "<a href='update_custom_property.php?status={$actionStatusCode}&custom_property_id={$row['custom_property_id']}' class='btn btn-primary'>Set as {$actionStatus}</a>";
                        }
                    }
