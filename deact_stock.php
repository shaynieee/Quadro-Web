    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            foreach ($statuses as $status => $status_code) {
                ...
                // Fetch and display users based on status
                $query = "SELECT * FROM stocks WHERE stock_status = '{$status_code}'";
                $result = mysqli_query($conn, $query);

                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<div class='user-details'>";
                    echo "<p>Stock Id: {$row['stock_id']}</p>";
                    echo "<p>Stock Quantity: {$row['stock_qty']}</p>";
                    ...
                    foreach ($statuses as $actionStatus => $actionStatusCode) {
                        if ($status !== $actionStatus) {
                            echo "<a href='update_stock_status.php?status={$actionStatusCode}&stock_id={$row['stock_id']}' class='btn btn-primary'>Set as {$actionStatus}</a>";
                        }
                    }
