    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            foreach ($statuses as $status => $status_code) {
                ...
                // Fetch and display users based on status
                $query = "SELECT * FROM pricing WHERE price_status = '{$status_code}'";
                $result = mysqli_query($conn, $query);

                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<div class='user-details'>";
                    echo "<p>Price Id: {$row['price_id']}</p>";
                    ...
                    foreach ($statuses as $actionStatus => $actionStatusCode) {
                        if ($status !== $actionStatus) {
                            echo "<a href='update_price_status.php?status={$actionStatusCode}&price_id={$row['price_id']}' class='btn btn-primary'>Set as {$actionStatus}</a>";
                        }
                    }
